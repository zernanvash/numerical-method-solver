...
// ── CRT colour palette (amber phosphor) ──────────────────────────────────────
static const QColor BG        ( 10,  8,  0);
static const QColor COL_GRID  ( 50, 38,  0, 90);
static const QColor COL_AXIS  (100, 76,  0, 180);
static const QColor COL_CURVE (255,176,  0);
static const QColor COL_SCAN  (255,210, 80);   // bright sweep line
static const QColor COL_BISECT(180,255,100);   // bisection interval
static const QColor COL_TANG  (100,200,255);   // newton tangent
static const QColor COL_SECANT(255,200, 80);   // secant chord
static const QColor COL_ROOT  (255,255,  0);   // final root dot
static const QColor COL_DIM   ( 90, 68,  0);
...
void GraphWidget::rebuildGraphBuffer() {
    ...
    applyScanlineRows(m_graphBuffer);
    applyDither(m_graphBuffer);
    applyVignette(m_graphBuffer);
...
void GraphWidget::drawBisectionStep(QPainter& p, int n) {
    ...
    QPen ep(COL_BISECT, 1, Qt::DashLine);
    ep.setColor(QColor(COL_BISECT.red(), COL_BISECT.green(), COL_BISECT.blue(),
...
void GraphWidget::drawNewtonStep(QPainter& p, int n) {
    ...
    QColor tc(COL_TANG.red(), COL_TANG.green(), COL_TANG.blue(),
              static_cast<int>(alpha * 200));
...
void GraphWidget::drawSecantStep(QPainter& p, int n) {
    ...
    QColor sc(COL_SECANT.red(), COL_SECANT.green(), COL_SECANT.blue(),
              static_cast<int>(alpha * 200));
...
void GraphWidget::drawRootMarker(QPainter& p, int n) {
    ...
    for (int r = 8; r >= 2; r -= 2) {
        ...
        p.setPen(Qt::NoPen);
        p.setBrush(QColor(COL_ROOT.red(), COL_ROOT.green(), COL_ROOT.blue(), alpha));
...
void GraphWidget::drawScanline(QPainter& p) {
    ...
    for (int g = 1; g <= 4; ++g) {
        ...
        p.setPen(QPen(QColor(COL_SCAN.red(), COL_SCAN.green(), COL_SCAN.blue(), alpha), 1));
